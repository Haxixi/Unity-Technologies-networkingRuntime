...
namespace UnityEditor
{
    ...
    public class NetworkManagerHUDEditor : Editor
    {
        ...
        void ShowMatchMakerInfo()
        {
            ...
            EditorGUILayout.LabelField("MatchMaker Host:", m_Manager.matchHost);
            EditorGUILayout.LabelField("MatchMaker Port:", m_Manager.matchPort.ToString());
            m_Manager.matchName = EditorGUILayout.TextField("Room Name:", m_Manager.matchName);
            m_Manager.matchSize = (uint)EditorGUILayout.IntField("Room Max Size:", (int)m_Manager.matchSize);
            EditorGUILayout.LabelField("Uri:", m_Manager.matchMaker.baseUri.ToString());
            if (m_Manager.matchInfo != null)
            {
                EditorGUILayout.LabelField("match", m_Manager.matchInfo.ToString());
            }
            EditorGUILayout.EndVertical();
            EditorGUI.indentLevel -= 1;
        }
